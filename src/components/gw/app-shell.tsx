  ...
  return (
    <div className="min-h-screen bg-background">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col bg-navy text-navy-foreground lg:flex">
        <div className="border-b border-navy-border px-6 py-5">
          <GridLogo tone="dark" />
        </div>
        <nav className="flex-1 space-y-1 p-3">
          {nav.map(({ to, label, icon: Icon }) => (
            <Link
              ...
              to={to}
              className={cn(
                "flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-navy-foreground/65 transition",
                path === to
                  ? "bg-primary text-primary-foreground"
                  : "hover:bg-navy-muted hover:text-white",
              )}
            >
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-[86%] max-w-72 bg-navy p-4 text-white transition-transform duration-200 lg:hidden",
          ...
          {nav.map(({ to, label, icon: Icon }) => (
            <Link
              ...
              className={cn(
                ...
                path === to ? "bg-primary text-primary-foreground" : "text-white/70",
              )}
            >
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-background/90 px-4 backdrop-blur-xl sm:px-6">
          ...
          <div className="hidden items-center gap-2 text-sm text-muted-foreground lg:flex">
            ...
            248 meters reporting live
          </div>
          <div className="flex items-center gap-2">
            <Button size="sm" className="hidden sm:inline-flex" onClick={() => setSimulate(true)}>
        ...
        <button
          ...
          className="fixed bottom-5 right-5 z-30 grid h-14 w-14 place-items-center rounded-full bg-primary text-primary-foreground shadow-lift sm:hidden"
          aria-label="Simulate meter report"
        >
          <Zap />
        </button>
      </div>
